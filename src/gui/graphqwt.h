...
class GraphQwt : public QWidget
{
  Q_OBJECT
  public:
    GraphQwt( QWorkspace *parent=0, const char *name=0,Spectrum *spec=0); //a GraphQwt of a normal spectrum
    GraphQwt( QWorkspace *parent, const char *name,Multispectrum *mspec); //a GraphQwt of a multispectrum
    ~GraphQwt();
    void reinit();
    void Init();
    void addGraphQwt(Spectrum* spec);
    void removelastGraphQwt();
    void setxlabel(const char* xl);
    void setylabel(const char* yl);
    void updateGraphQwt(int layer,Spectrum* spec);
...
private:
    ...
    QRect r; //the rectangle of the GraphQwt region
    QRect grabrect;
